...
const titles={creator:"Educational Activity Book Creator",projects:"Saved Projects",templates:"Template Library"};
...
$("#exportJson").addEventListener("click",()=>current&&download("activity-book.json",JSON.stringify(current,null,2),"application/json"));$("#exportTxt").addEventListener("click",()=>{if(!current)return;let t=`${current.book_title}\n${current.subtitle}\n\n${current.description}\n\n`;current.pages.forEach(p=>t+=`PAGE ${p.page_number}: ${p.title}\n${p.instruction}\nContent: ${p.content_items.join(", ")}\nImage prompt: ${p.image_prompt}\nAnswer: ${p.answer}\n\n`);download("activity-book.txt",t)});
